...
This vignette will work with data from the *social-distancing/v2/* SafeGraph Amazon Web Services bucket (see the [SafeGraph Catalog](catalog.safegraph.io) for more information), which could also be downloaded using `safegraph_aws()`. 
...
We will read in the distancing data using the `read_distancing()` function. This relies on the files in the `social-distancing/v2` bucket in the Amazon Web Services bucket, which you can download using `safegraph_aws()` (or using the AWS CLI). See the [SafeGraph Catalog](catalog.safegraph.io).
